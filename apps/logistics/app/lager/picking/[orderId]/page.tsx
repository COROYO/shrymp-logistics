...
import { loadPickScanItems } from "@/server/warehouse/pick-scan";
import { StartPickingButton, CancelPickingButton } from "./client-buttons";
import { PickScanVerifier } from "./pick-scan-verifier";
...
/**
 * Collapse line items that share the same title + sku into one picklist row.
 * Shopify can split one logical product across several line items; the picker
 * needs a single combined position. Quantities are summed and Charge
 * allocations merged by charge number.
 */
function mergeLineItems(items: LineItemDisplay[]): LineItemDisplay[] {
  const groups = new Map<string, LineItemDisplay>();
  const order: string[] = [];

  for (const li of items) {
    const key = `${li.productTitle} ${li.variantTitle} ${li.sku ?? ""}`;
    ...
    g.allocations.push(...li.allocations);
  }

  for (const g of groups.values()) {
    // Merge allocations sharing a charge number, sum quantities, keep FEFO sort.
    const byCharge = new Map<string, AllocationLine>();
    for (const a of g.allocations) {
      const cur = byCharge.get(a.chargeNumber);
      if (cur) cur.qty += a.qty;
      else byCharge.set(a.chargeNumber, { ...a });
    }
    g.allocations = [...byCharge.values()].sort((a, b) => {
...
async function loadOrder(orderId: string) {
  const db = adminDb();
  const firstSnap = await db.collection(Collections.Orders).doc(orderId).get();
  if (!firstSnap.exists) return null;
  const shopId = (firstSnap.data() as Order).shop_id;
  if (!shopId) return null;
  // Tenant gate + context so batch config (loadLagerConfig) resolves the shop.
  await assertShopAccessibleForPage(shopId, `/lager/picking/${orderId}`);
  await runWithTenantAsync(shopId, () =>
    releaseUnshippableBatchAssignments(orderId),
  );
  ...
  const rawLineItems: LineItemDisplay[] = order.line_items.map((li) => {
  ...
  const lineItems = mergeLineItems(rawLineItems);

  const [batchesEnabled, pickItems] = await Promise.all([
    isBatchesEnabled(shopId),
    runWithTenantAsync(shopId, () => loadPickScanItems(order, shopId)),
  ]);

  const binByVariant = new Map<string, { code: string; name: string }>();
  for (const it of pickItems) {
    if (it.binCode) {
      binByVariant.set(it.variantId, { code: it.binCode, name: it.binName ?? "" });
    }
  }

  return { order, lineItems, batchesEnabled, pickItems, binByVariant };
}
...
export default async function PickingDetailPage({
  ...
  if (!result) notFound();
  const { order, lineItems, batchesEnabled, pickItems, binByVariant } = result;
  ...
  return (
    <div className="space-y-8">
          ...
          <table className="table-brand">
            <thead>
              <tr>
                ...
                {batchesEnabled ? <th>{t("items.charge")}</th> : null}
              </tr>
            </thead>
            <tbody>
              {lineItems.map((li) => {
                const bin = binByVariant.get(li.variantId);
                return (
                <tr key={li.id}>
                  <td>
                  ...
                  <td>
                    {bin ? (
                      <span className="inline-flex items-center rounded-md bg-brand-cream px-2 py-0.5 font-mono text-xs font-bold text-brand-navy">
                        {bin.code}
                      </span>
                    ) : (
      ...
      {isPicking ? (
        <PickScanVerifier items={pickItems} />
      ) : null}
